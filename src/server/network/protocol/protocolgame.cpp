...
void ProtocolGame::parseFightModes(NetworkMessage &msg) {
	msg.getByte(); // fight mode byte — ignored, server always locks to FIGHTMODE_ATTACK
	uint8_t rawChaseMode = msg.getByte(); // 0 - stand while fighting, 1 - chase opponent
	uint8_t rawSecureMode = msg.getByte(); // 0 - can't attack unmarked, 1 - can attack unmarked
	uint8_t rawPvpMode = msg.getByte(); // pvp mode

	g_game().playerSetFightModes(player->getID(), FIGHTMODE_ATTACK, rawChaseMode != 0, rawSecureMode != 0, rawPvpMode);
}
...
void ProtocolGame::resendPvpSquares() {
	...
	for (const auto &spectator : Spectators().find<Player>(player->getPosition(), true, 0, 0, 0, 0, false)) {
		...
		if (sp && sp != player && !sp->pvpAggressors.empty()) {
			const bool involved = player->hasPvpAggressor(sp->getGUID());
			sendCreatureSquare(sp, involved ? SQ_COLOR_YELLOW : SQ_COLOR_BROWN, 2);
		}
	}
}
...
void ProtocolGame::sendMapDescription(const Position &pos) {
	...
	GetMapDescription(pos.x - MAP_SEND_VIEW_PORT_X, pos.y - MAP_SEND_VIEW_PORT_Y, pos.z, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, msg);
	writeToOutputBuffer(msg);
	for (const auto &c : m_pendingPvpSquares) {
		const auto &cp = c->getPlayer();
		const bool involved = cp && (player == cp || player->hasPvpAggressor(cp->getGUID()));
		sendCreatureSquare(c, involved ? SQ_COLOR_YELLOW : SQ_COLOR_BROWN, 2);
	}
	...
	if (creature != player) {
		...
		if (const auto &creaturePlayer = creature->getPlayer()) {
			if (!creaturePlayer->pvpAggressors.empty()) {
				const bool involved = player->hasPvpAggressor(creaturePlayer->getGUID());
	...
	if (creature == player) {
		if (oldStackPos >= 10) {
			...
			if (oldPos.y > newPos.y) { // north, for old x
				msg.addByte(0x65);
				GetMapDescription(oldPos.x - MAP_SEND_VIEW_PORT_X, newPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z, (MAP_SEND_VIEW_PORT_X + 1) * 2, 1, msg);
			} else if (oldPos.y < newPos.y) { // south, for old x
				msg.addByte(0x67);
				GetMapDescription(oldPos.x - MAP_SEND_VIEW_PORT_X, newPos.y + (MAP_SEND_VIEW_PORT_Y + 1), newPos.z, (MAP_SEND_VIEW_PORT_X + 1) * 2, 1, msg);
			}

			if (oldPos.x < newPos.x) { // east, [with new y]
				msg.addByte(0x66);
				GetMapDescription(newPos.x + (MAP_SEND_VIEW_PORT_X + 1), newPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z, 1, (MAP_SEND_VIEW_PORT_Y + 1) * 2, msg);
			} else if (oldPos.x > newPos.x) { // west, [with new y]
				msg.addByte(0x68);
				GetMapDescription(newPos.x - MAP_SEND_VIEW_PORT_X, newPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z, 1, (MAP_SEND_VIEW_PORT_Y + 1) * 2, msg);
			}
			writeToOutputBuffer(msg);
			for (const auto &c : m_pendingPvpSquares) {
				const auto &cp = c->getPlayer();
...
void ProtocolGame::MoveUpCreature(NetworkMessage &msg, const std::shared_ptr<Creature> &creature, const Position &newPos, const Position &oldPos) {
	...
	if (newPos.z == MAP_INIT_SURFACE_LAYER) {
		int32_t skip = -1;
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, 5, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 3, skip); //(floor 7 and 6 already set)
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, 4, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 4, skip);
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, 3, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 5, skip);
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, 2, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 6, skip);
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, 1, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 7, skip);
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, 0, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 8, skip);
	...
	// underground, going one floor up (still underground)
		...
		int32_t skip = -1;
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, oldPos.getZ() - 3, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, 3, skip);
	...
	// west
	msg.addByte(0x68);
	GetMapDescription(oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - (MAP_SEND_VIEW_PORT_Y - 1), newPos.z, 1, (MAP_SEND_VIEW_PORT_Y + 1) * 2, msg);

	// north
	msg.addByte(0x65);
	GetMapDescription(oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z, (MAP_SEND_VIEW_PORT_X + 1) * 2, 1, msg);
}
...
void ProtocolGame::MoveDownCreature(NetworkMessage &msg, const std::shared_ptr<Creature> &creature, const Position &newPos, const Position &oldPos) {
	...
	if (newPos.z == MAP_INIT_SURFACE_LAYER + 1) {
		...
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, -1, skip);
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z + 1, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, -2, skip);
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z + 2, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, -3, skip);
	...
	// going further down
		...
		int32_t skip = -1;
		GetFloorDescription(msg, oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y - MAP_SEND_VIEW_PORT_Y, newPos.z + MAP_LAYER_VIEW_LIMIT, (MAP_SEND_VIEW_PORT_X + 1) * 2, (MAP_SEND_VIEW_PORT_Y + 1) * 2, -3, skip);
	...
	// east
	msg.addByte(0x66);
	GetMapDescription(oldPos.x + MAP_SEND_VIEW_PORT_X + 1, oldPos.y - (MAP_SEND_VIEW_PORT_Y + 1), newPos.z, 1, ((MAP_SEND_VIEW_PORT_Y + 1) * 2), msg);

	// south
	msg.addByte(0x67);
	GetMapDescription(oldPos.x - MAP_SEND_VIEW_PORT_X, oldPos.y + (MAP_SEND_VIEW_PORT_Y + 1), newPos.z, ((MAP_SEND_VIEW_PORT_X + 1) * 2), 1, msg);
}
